...
7. **`research_competitor_watch()`** — a dedicated, mandatory per-name check across a **tiered** competitor list: the priority `CORE_COMPETITORS` (the most direct threats — Resy, OpenTable, SevenRooms, Tock, TheFork, Dorsia, Appointment Trader, Ambl, …) checked deeply **every run**, plus a rotating chunk of the long tail (`competitor_watch_list`, `TAIL_CHUNK_SIZE`) so the whole ~89-name list (`data/competitors.json` / `SEED_COMPETITORS`) gets covered over a few runs without asking the model to check 89 names in one call. Explicitly instructed that completeness beats brevity.
